...
# ── Overview ──────────────────────────────────────────────────────────────────
# Paper 2 — RQ3 (macro-economic framing):
# Among older adults who intended to move at baseline, which factors are
# associated with NOT having relocated after three years?
#
# Framing: Structural and macro-economic barriers (housing supply constraints,
# financial obstacles) rather than individual-level factors.
#
# Subset:  Intenders — VAR024 %in% c(1, 2)
# Outcome: relocated_f. ORs < 1 = reduced odds of relocation.
#
# Models:
#   M1 — age + sex + intention_level
#   M2 — M1 + housing factors
#   M3 — M1 + obstacles
#   M4 — M1 + housing + obstacles (full model)
#
# Outputs:
#   figures/RQ3_blocked_forest.png
#   tables/RQ3_blocked_intender_sample.csv
#   tables/RQ3_blocked_obstacle_prevalence.csv
#   tables/RQ3_blocked_all_coefficients.csv
#   tables/RQ3_blocked_model_fit.csv
#   tables/RQ3_blocked_lrt.csv
#   models/RQ3_blocked_m1.rds … RQ3_blocked_m4.rds
...
m2_rq3 <- glm(relocated_f ~ intention_level + age + sex + srh +
                housing_suitability + home_satisfaction + neighbourhood_cohesion,
              data = dat_m, family = binomial)
m3_rq3 <- glm(relocated_f ~ intention_level + age + sex + srh +
                any_obstacle + obs_financial + obs_supply + obs_energy +
                obs_own_health + obs_partner_health + obs_dependents + obs_bulky,
              data = dat_m, family = binomial)
m4_rq3 <- glm(relocated_f ~ intention_level + age + sex + srh +
                housing_suitability + home_satisfaction + neighbourhood_cohesion +
                any_obstacle + obs_financial + obs_supply + obs_energy +
                obs_own_health + obs_partner_health + obs_dependents + obs_bulky,
              data = dat_m, family = binomial)
...
write_csv(intender_sample, "paper/paper2/tables/RQ3_blocked_intender_sample.csv")
write_csv(obstacle_prev,   "paper/paper2/tables/RQ3_blocked_obstacle_prevalence.csv")
write_csv(all_coef,        "paper/paper2/tables/RQ3_blocked_all_coefficients.csv")
write_csv(model_fit,       "paper/paper2/tables/RQ3_blocked_model_fit.csv")
write_csv(lrt,             "paper/paper2/tables/RQ3_blocked_lrt.csv")

write_rds(m1_rq3, "paper/paper2/models/RQ3_blocked_m1.rds")
write_rds(m2_rq3, "paper/paper2/models/RQ3_blocked_m2.rds")
write_rds(m3_rq3, "paper/paper2/models/RQ3_blocked_m3.rds")
write_rds(m4_rq3, "paper/paper2/models/RQ3_blocked_m4.rds")

# Forest plot — M3 (obstacles model)
...
p_forest <- ggplot(forest_dat,
  ...
  labs(x = "Odds ratio (log scale)", y = NULL, colour = NULL,
       title   = "RQ3 (Paper 2): Predictors of relocation among intenders (M3)",
...
ggsave("paper/paper2/figures/RQ3_blocked_forest.png", p_forest,

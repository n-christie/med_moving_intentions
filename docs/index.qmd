...
**Objective:** To examine whether self-reported moving intentions and housing-related factors at baseline predict actual relocation over a three-year period, to identify factors associated with non-relocation among those who intended to move, and to examine which factors predict unexpected relocation among those who did not intend to move.

**Methods:** Prospective longitudinal survey study with three time points (T1: 2021, T2: 2022, T3: 2024) among adults aged 55 or older registered on housing company interest lists in Sweden (N = 1,961). Binary logistic regression was used to model the probability of relocation, with models built sequentially from intentions and demographics through housing factors, perceived obstacles, and housing tenure. Cox proportional hazards models were fitted using exact relocation dates to account for varying follow-up length; a stratified Cox model was used to address a proportional hazards violation for intention timeframe. Latent profile analysis (LPA) was used to identify person-centered profiles defined by intention urgency, home satisfaction, housing suitability, self-rated health, age, and neighborhood cohesion.
...
**Conclusions:** Moving intentions are the strongest predictor of relocation, but housing dissatisfaction independently predicts relocation across the intention spectrum and is the primary driver of unexpected moves among those who did not intend to relocate. Latent profile analysis identified five person-centered profiles; the environmentally pressured profile (n = 134, 47% relocated) was characterized by markedly low home satisfaction, low neighborhood cohesion, and elevated proportions of renters and single-person households, illustrating that person-environment mismatch can drive relocation even at moderate levels of stated intention. The substantial gap between intention and action among intenders, combined with the renter vulnerability effect among non-intenders, points to structural and market-level constraints that individual-level surveys cannot fully capture. Findings have implications for housing planning and anticipatory service provision for older adults.
...
**(c)** To examine which factors predict unexpected relocation among older adults who did not intend to move at baseline.
...
|---|---|---|
| Outcome validation | Cross-tabulate self-reported vs register-derived relocation (address change); re-run RQ1–RQ4 with register outcome | Location register |
| Attrition analysis | Classify T3 non-respondents (n = 545) as deceased, dropout, or censored; refit survival models with corrected censoring | Mortality register |
| Health trajectory | Add objective comorbidity index (Charlson/Elixhauser) to models; test whether the null health effect reflects measurement limitations in self-rated health | NPR + prescription register |
...
| Objective housing accessibility | Replace or supplement housing suitability (currently shows collinearity with satisfaction) with building-level measures (elevator, storey, building year) | Building/apartment register |
